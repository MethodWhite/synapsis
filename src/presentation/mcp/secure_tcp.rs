...
impl SecureTcpServer {
    ...
    pub fn new(mcp_server: Arc<McpServer>, addr: &str) -> std::io::Result<Self> {
        ...
        let crypto_provider: Arc<dyn CryptoProvider> = Arc::new(SynapsisPqcProvider::new()) as Arc<dyn CryptoProvider>;
        Ok(Self {
            mcp_server,
            listener,
            crypto_provider,
    ...
    pub fn run(&self) -> std::io::Result<()> {
        eprintln!("[MCP Secure TCP] Server listening on {}", self.listener.local_addr()?);
        ...
        for stream in self.listener.incoming() {
            match stream {
                Ok(stream) => {
                    ...
                    thread::spawn(move || {
                        if let Err(e) = handle_secure_connection(mcp_server, crypto_provider, stream) {
...
fn handle_secure_connection(
    ...
    let shared_secret = match perform_kyber_handshake(crypto_provider.as_ref(), &stream) {
        Ok(secret) => secret,
        Err(e) => {
            eprintln!("[MCP Secure TCP] Handshake failed with {}: {}", peer_addr, e);
            return Ok(());
        }
    };
    
    eprintln!("[MCP Secure TCP] Secure channel established with {}", peer_addr);
        ...
        match reader.read_line(&mut line) {
            ...
            Ok(_) => {
                ...
                let decrypted = match decrypt_message(crypto_provider.as_ref(), &aes_key, line) {
                    Ok(msg) => msg,
                    Err(e) => {
                        eprintln!("[MCP Secure TCP] Decryption error from {}: {}", peer_addr, e);
                        break;
                    }
                };
                ...
                if let Some(response) = mcp_server.handle_message(&decrypted) {
                    // Encrypt response
                    let encrypted = match encrypt_message(crypto_provider.as_ref(), &aes_key, &response) {
                        Ok(enc) => enc,
                        Err(e) => {
                            eprintln!("[MCP Secure TCP] Encryption error to {}: {}", peer_addr, e);
                            break;
                        }
                    };
...
fn perform_kyber_handshake(
    ...
    let client_pk = base64::decode(client_pk_line)
    ...
    let mut writer = stream.try_clone().map_err(|e| format!("Clone stream for write: {}", e))?;
    let response = format!(
        "{} {}\n",
...
fn decrypt_message(
    ...
    String::from_utf8(plaintext)
        .map_err(|e| format!("Invalid UTF-8 plaintext: {}", e))
}
...
fn perform_client_handshake(
    ...
    writeln!(writer, "{}", base64::encode(&client_pk))
    ...
    let server_pk = base64::decode(parts[0])

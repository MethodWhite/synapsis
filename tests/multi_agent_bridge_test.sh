...
response=$(echo '{"jsonrpc":"2.0","method":"ping","params":{},"id":1}' | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
QWEN_RESPONSE=$(echo '{"jsonrpc":"2.0","method":"session_register","params":{"arguments":{"agent_type":"qwen-code-test","project":"bridge-test"}},"id":2}' | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
OPENCODE_RESPONSE=$(echo '{"jsonrpc":"2.0","method":"session_register","params":{"arguments":{"agent_type":"opencode-test","project":"bridge-test"}},"id":3}' | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
if [ -n "$QWEN_SESSION" ]; then
    SAVE_RESPONSE=$(echo "{\"jsonrpc\":\"2.0\",\"method\":\"chunk_create\",\"params\":{\"arguments\":{\"project\":\"bridge-test\",\"title\":\"qwen-context\",\"content\":\"qwen-code-test-data-$(date +%s)\"}},\"id\":4}" | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
if [ -n "$OPENCODE_SESSION" ]; then
    READ_RESPONSE=$(echo '{"jsonrpc":"2.0","method":"chunk_get","params":{"arguments":{"project":"bridge-test"}},"id":5}' | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
TASK_RESPONSE=$(echo '{"jsonrpc":"2.0","method":"task_create","params":{"arguments":{"project":"bridge-test","task_type":"test","payload":"multi-agent-test","priority":10}},"id":6}' | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
CLAIM1=$(echo "{\"jsonrpc\":\"2.0\",\"method\":\"task_claim\",\"params\":{\"arguments\":{\"session_id\":\"$QWEN_SESSION\",\"task_type\":\"test\"}},\"id\":7}" | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
CLAIM2=$(echo "{\"jsonrpc\":\"2.0\",\"method\":\"task_claim\",\"params\":{\"arguments\":{\"session_id\":\"$OPENCODE_SESSION\",\"task_type\":\"test\"}},\"id\":8}" | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
...
HB1=$(echo "{\"jsonrpc\":\"2.0\",\"method\":\"agent_heartbeat\",\"params\":{\"arguments\":{\"session_id\":\"$QWEN_SESSION\",\"task\":\"test\"}},\"id\":9}" | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
HB2=$(echo "{\"jsonrpc\":\"2.0\",\"method\":\"agent_heartbeat\",\"params\":{\"arguments\":{\"session_id\":\"$OPENCODE_SESSION\",\"task\":\"test\"}},\"id\":10}" | nc -q1 $SYNAPSIS_SERVER 2>/dev/null)
